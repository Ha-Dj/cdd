...
from insert_module.SE_block import fusion_SE_block
...
class Restormer_Decoder(nn.Module):
    def __init__(self,
        ...
        self.f_se_block = fusion_SE_block(dim, 16)
...
if __name__ == '__main__':
    ...
    width = 128
    window_size = 8
    input1 = torch.rand(1, 1, height, width)
    modelE = Restormer_Encoder()
    modelE(input1)

    inp_img = torch.randn(1, 1, 32, 32)  # 1个1通道的32x32的随机张量
    base_feature = torch.randn(1, 64, 32, 32)  # 1个64通道的32x32的随机张量
    detail_feature = torch.randn(1, 64, 32, 32)  # 1个64通道的32x32的随机张量
    modelD = Restormer_Decoder()
    print(base_feature.shape)
    modelD(inp_img, base_feature, detail_feature)
    print(detail_feature.shape)
